...
public class BoardDAO {
	...
	public List<MyBoardVO> svcSelect() { 
		...
		System.out.println("svcSelect 호출");
		List<MyBoardVO> alist = new ArrayList<MyBoardVO>();
...
//		System.out.println("--내부 select * done--");
	...
	public int svcInsert(MyBoardVO bvo) {
		System.out.println("svcInsert 호출");
		//Connection conn = null;
		...
		int rows = 0;
		//TODO
		String sql = "insert into myboard(bseq, title, contents, regid) values(seq_myboard.nextval, ?, ?, ?)";
		
		rows = session.insert(sql, bvo);
		session.commit();
//		try {
//			conn = ds.getConnection();
//			pstmt = conn.prepareStatement(sql);
